        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    @if($errors->has('assigned_staff'))
                        <div class="invalid-feedback">
                            {{ $errors->first('assigned_staff') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.user.fields.company_name_helper') }}</span>
                </div>
                <div class="form-group" id="hide_display_staff_selector">
                    <label for="assigned_staff_email">{{ trans('cruds.user.fields.assigned_staff_email') }}</label>
                    <select class="form-control {{ $errors->has('assigned_staff_email') ? 'is-invalid' : '' }}" type="text" name="assigned_staff_email" id="assigned_staff_email" value="{{ old('assigned_staff_email', '') }}">
                        ...
                        @if($all_staff)
                            @foreach($all_staff as $key => $staff_name)
                                <option id="{{ $staff_name->id }}">{{ $staff_name->email }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('principal_place_of_business') ? 'is-invalid' : '' }}" type="text" name="principal_place_of_business" id="principal_place_of_business" value="{{ old('principal_place_of_business', '') }}">
                    @if($errors->has('principal_place_of_business'))
                        <div class="invalid-feedback">
                            {{ $errors->first('principal_place_of_business') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.user.fields.principal_place_of_business_helper') }}</span>
                </div>
